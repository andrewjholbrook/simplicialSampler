...
df <- read_table2("inst/output/rwmComparison.txt",
                  col_names = FALSE)
mtm <- read_table2("inst/output/mtmUpdatedRuns.txt",
                   col_names = FALSE)
df <- df[,-ncol(df)]
mtm <- mtm[mtm$X2=="spherical",-ncol(mtm)]
mtm <- mtm[,-2]
df[,8:10] <- mtm[,2:4]
df$X2 <- df$X2 / df$X5
df$X3 <- df$X3 / df$X6
...
gg4 <- ggplot(df, aes(x=Dimension,y=`Relative improvement`,color=`Criterion:`, shape=`Statistic:`)) +
  geom_point() +
  geom_smooth() +
  scale_y_continuous(trans = "log2",breaks = c(0.5,1,2,4,8,16,32),limits=c(0.25,36)) +
...
mtm <- read_table2("inst/output/mtmUpdatedRuns.txt",
                   col_names = FALSE)
df <- df[17:nrow(df),-ncol(df)]
mtm <- mtm[mtm$X2=="diag",-ncol(mtm)]
mtm <- mtm[,-2]
df[,8:10] <- mtm[,2:4]
df$X2 <- df$X2 / df$X8
df$X3 <- df$X3 / df$X9
...
mtm <- read_table2("inst/output/mtmUpdatedRuns.txt",
                   col_names = FALSE)
df <- df[11:nrow(df),-ncol(df)]
mtm <- mtm[mtm$X2=="fullyIll",-ncol(mtm)]
mtm <- mtm[,-2]
df[,8:10] <- mtm[,2:4]
df$X2 <- df$X2 / df$X8
df$X3 <- df$X3 / df$X9
...
ggsave(ggarrange(gg,gg2,gg3,gg4,gg5,gg6,ncol = 3,nrow=2,common.legend = TRUE,
                 legend="bottom"),
       file="inst/figures/rwmComparisonFig.pdf",
       width = 12,height = 7)
